...
export const useMeQuery = () => {
  ...
  return useQuery<User, Error>([API_ENDPOINTS.ME], userClient.me, {
    ...
    onSuccess: () => {
      // Always redirect to dashboard if on verifyLicense page
      if (router.pathname === Routes.verifyLicense && router.pathname !== Routes.dashboard) {
    ...
    onError: (err) => {
      if (axios.isAxiosError(err)) {
        if (err.response?.status === 417) {
          // If license not verified, redirect to dashboard (home page) instead of verifyLicense
    ...
    // Fallback: if stuck on verifyLicense, force redirect to dashboard after mount
    onSettled: () => {
      if (router.pathname === Routes.verifyLicense && router.pathname !== Routes.dashboard) {
        router.replace(Routes.dashboard);
      }
...
export const useResendVerificationEmail = () => {
  ...
  return useMutation(userClient.resendVerificationEmail, {
    onSuccess: () => {
      toast.success(t('common:TEZVYAPAR_MESSAGE.EMAIL_SENT_SUCCESSFUL'));
    },
    onError: () => {
      toast(t('common:TEZVYAPAR_MESSAGE.EMAIL_SENT_FAILED'));
    },
  });
};
...
export const useLicenseKeyMutation = () => {
  ...
  return useMutation(userClient.addLicenseKey, {
    ...
    onError: () => {
      toast.error(t('common:TEZVYAPAR_MESSAGE.INVALID_LICENSE_KEY'));
    },
  });
};
