...
class EnsureEmailIsVerified
{
    ...
    public function handle($request, Closure $next, $redirectToRoute = null)
    {
        ...
        $useMustVerifyLicense = isset($setting->options['app_settings']['trust']) ? $setting->options['app_settings']['trust'] : true; // Tezvyapar license check
        $localLicense = getConfig();
        $useLocalLicense = isset($localLicense['trust']) ? $localLicense['trust'] : true; // Tezvyapar local license
        $useMustVerifyEmail = isset($setting->options['useMustVerifyEmail']) ? $setting->options['useMustVerifyEmail'] : false; // Tezvyapar email verification
